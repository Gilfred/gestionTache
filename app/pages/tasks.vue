        ...
        <li v-for="task in filteredTasks" :key="task.id" class="px-4 py-4 sm:px-6 hover:bg-gray-50">
          <div class="flex items-center justify-between">
            ...
            <div class="ml-4 flex-shrink-0 flex gap-2">
              <button @click="editTask(task)" class="text-indigo-600 hover:text-indigo-900">Modifier</button>
              <button @click="deleteTask(task.id)" class="text-red-600 hover:text-red-900">Supprimer</button>
            </div>
          </div>
        </li>
      </ul>
